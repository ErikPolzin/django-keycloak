...
class Server(models.Model):
    """Keycloak server (just the URL really)."""
...
class Realm(models.Model):
    ...
    @cached_property
    def openid_api_client(self) -> KeycloakOpenID:
        """Return keykloak OpenID client."""
        return KeycloakOpenID(
            self.server.url, self.name, self.client.client_id, self.client.secret
        )
...
class Client(models.Model):
    ...
    service_account_profile = models.OneToOneField(
        "OpenIdConnectProfile", on_delete=models.CASCADE, null=True
    )
...
class OpenIdConnectProfile(models.Model):
    ...
    realm = models.ForeignKey(
        Realm, related_name="openid_profiles", on_delete=models.CASCADE
    )
    ...
    @classmethod
    def from_code(
        cls, realm: Realm, code: str, redirect_uri: str = ""
        ...
        token_response = realm.openid_api_client.token(
            code=code, redirect_uri=redirect_uri, grant_type="authorization_code"
        )
        oidc_profile = cls.from_token_response(realm, token_response)
        oidc_profile.update_tokens(token_response, initiate_time)
        return oidc_profile

    @classmethod
    def from_credentials(
        cls, realm: Realm, username: str, password: str, redirect_uri: str = ""
    ) -> "OpenIdConnectProfile":
    ...
    @classmethod
    def from_token_response(cls, realm: Realm, token_response: dict) -> "OpenIdConnectProfile":
        """Generate an OIDC profile from an auth server response."""
        token_response_key = (
            "id_token" if "id_token" in token_response else "access_token"
        )
        token_object = realm.openid_api_client.decode_token(
        ...
        with transaction.atomic():
            ...
            oidc_profile, _ = OpenIdConnectProfile.objects.update_or_create(
                sub=token_object["sub"], defaults={"realm": realm, "user": user}
            )
        return oidc_profile

    @property
    def api_client(self) -> KeycloakOpenID:
        """This profile's realm's OpenID API client."""
        return self.realm.openid_api_client
    ...
    def jwt(self):
        ...
        if not self.is_active:
            return None
        return self.api_client.decode_token(self.access_token)
    ...
    def get_active_access_token(self) -> str | None:
        ...
        if initiate_time > self.expires_before:
            # Refresh token
            token_response = self.api_client.refresh_token(
    ...
    def entitlement(self) -> dict:
        ...
        resource_id = self.realm.client.client_id
        # BUG: This fails with coded 405: Method Not Allowed?
        rpt = self.api_client.entitlement(access_token, resource_id)["rpt"]
        rpt_decoded = self.api_client.decode_token(rpt)
        return rpt_decoded
